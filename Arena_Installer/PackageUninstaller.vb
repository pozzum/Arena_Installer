...
Namespace PackageUninstaller
    Public Class Uninstall
        ...
        Private Class BGW_Instructions
            ...
            Public BackupDirectoryInfoList As List(Of DirectoryInfo)
        ...
        Public Sub BeginOperation(ByVal sourceDirectoryPath As String,
                                  ByVal BackupDirectoryPath As String,
            ...
            BackupSource = BackupDirectoryPath
            Try
                If String.IsNullOrEmpty(sourceDirectoryPath) OrElse sourceDirectoryPath.Trim = "" Then
                    Throw New ArgumentException("The directory path to uninstall cannot be null or empty.")

                ElseIf String.IsNullOrEmpty(BackupDirectoryPath) OrElse BackupDirectoryPath.Trim = "" Then
                    Throw New ArgumentException("The directory path to backup from cannot be null or empty.")

                ElseIf String.IsNullOrEmpty(targetBaseDirectoryPath) OrElse targetBaseDirectoryPath.Trim = "" Then
                    Throw New ArgumentException("The directory path to uninstall from cannot be null or empty.")

                Else
                    Dim sourceDI As New DirectoryInfo(sourceDirectoryPath)
                    Dim backupDI As New DirectoryInfo(BackupDirectoryPath)
                    ...
                    ElseIf Not backupDI.Exists Then
                        Throw New DirectoryNotFoundException("The backup directory path could not be located.")
                    ...
                    Else
                        ...
                        Dim instructions As New BGW_Instructions() With
                            ...
                            .BackupDirectoryInfoList = New List(Of DirectoryInfo)() From {backupDI},
            ...
            Try
                If e.Argument IsNot Nothing Then
                    ...
                    If instructions IsNot Nothing Then
                        ...
                        Dim sourcefileInfoList As New List(Of FileInfo)
                        Dim backupfileInfoList As New List(Of FileInfo)
                        Dim sourceDirectoryPaths As New List(Of String)
                        Dim backupDirectoryPaths As New List(Of String)
                        ...
                        For Each di As DirectoryInfo In instructions.SourceDirectoryInfoList
                            If _userCancel Then
                                bgw.CancelAsync()
                            Else
                                sourceDirectoryPaths.Add(di.FullName)
                                ...
                                If instructions.ParsingType = DirectoryParsing.IncludeSubfolders Then
                                    ...
                                    If list IsNot Nothing Then
                                        For Each dirPath As String In list
                                            sourceDirectoryPaths.Add(dirPath)
                                        Next
                                    End If

                                    sourceDirectoryPaths.Sort()
                                End If
                            End If
                        Next
                        For Each di As DirectoryInfo In instructions.BackupDirectoryInfoList
                            If _userCancel Then
                                bgw.CancelAsync()
                            Else
                                backupDirectoryPaths.Add(di.FullName)
                                ...
                                If instructions.ParsingType = DirectoryParsing.IncludeSubfolders Then
                                    ...
                                    If list IsNot Nothing Then
                                        For Each dirPath As String In list
                                            backupDirectoryPaths.Add(dirPath)
                                        Next
                                    End If

                                    backupDirectoryPaths.Sort()
                                End If
                            End If
                        Next
                        _totalDirectoryCount = sourceDirectoryPaths.Count
                        _currentOperation = OperationType.EnumeratingFiles

                        For Each dirPath As String In sourceDirectoryPaths
                            If _userCancel Then
                                bgw.CancelAsync()
                            Else
                            ...
                            End If
                        Next
                        For Each dirPath As String In backupDirectoryPaths
                            If _userCancel Then
                                bgw.CancelAsync()
                            Else
                                Dim files As String() = Directory.GetFiles(dirPath)
                        ...
                        _currentOperation = OperationType.Uninstalling

                        For Each fi As FileInfo In sourcefileInfoList
                            If _userCancel Then
                                bgw.CancelAsync()
                            Else
                                'Pozzum - I want this to not copy all of the folders just the top level folder name.
                                'so this will like make it matches and overwrites correctly.
                                Dim targetPath As String = instructions.TargetBaseDirectory.FullName & "\" &
                                    fi.FullName.Replace(OriginalSource, "")
                                'Should be the game folder
                                Dim targetDirectoryPath As String = GetDirectoryName(targetPath)
                                _currentFileCount += 1
                                _cumulativeFileSize += fi.Length
                                If File.Exists(fi.FullName.Replace(OriginalSource, BackupSource)) Then
                                    'MessageBox.Show("Copying: " & Path.GetFileName(targetPath))
                                    File.Copy(fi.FullName.Replace(OriginalSource, BackupSource), targetPath, True)
                                Else
                                    'MessageBox.Show("Deleting: " & Path.GetFileName(targetPath))
            ...
            Public ReadOnly Property CurrentActivity_String As String
                Get
                    Select Case _currentOperation
                        Case OperationType.Uninstalling
                            Return "Uninstalling Directories/Files"
